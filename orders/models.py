from django.db import models
from users.models import UserTemporary
from products.models import Product
# Create your models here.
class Order(models.Model):
    user_id = models.ForeignKey(UserTemporary, on_delete=models.PROTECT)
    order_date = models.DateField(auto_now_add=True)
    status = models.CharField(max_length=100)
    delivery_address = models.ForeignKey('UserTemporary', on_delete=models.CASCADE)
    def __str__(self):
        return f"{self.user_id}, date: {self.order_date},\
        status: {self.status}, address: {self.delivery_address}"
class DeliveryAddress(models.Model):
    user = models.ForeignKey(UserTemporary, on_delete=models.CASCADE)
    address = models.CharField(max_length=100)
    def __str__(self):
        return f"username: {self.user}, address: {self.address}"

class OrderItem(models.Model):
    order_id = models.ForeignKey(Order, on_delete=models.PROTECT)
    product_id = models.ForeignKey(Product, on_delete=models.CASCADE)
    quantity = models.IntegerField(max_length=100)
    price = models.DecimalField(max_digits=10, decimal_places=2)
    def __str__(self):
        return f"order: {self.order_id}, product: {self.product_id}, \
        quantity: {self.quantity}, price: {self.price}"